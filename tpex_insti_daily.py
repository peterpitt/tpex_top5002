...
# 用法：
#   python tpex_insti_daily.py --side buy --date "" --out out/tpex_buy.csv
...
def main():
    ...
    # 確保輸出資料夾存在（避免 runner 沒先 mkdir 的情況）
    out_dir = os.path.dirname(args.out) or "."
    os.makedirs(out_dir, exist_ok=True)

    day, rows = fetch_daily(args.side, args.date)

    # 不中斷 CI：非交易日/無資料 → 輸出表頭、exit 0
    if not rows:
        print("⚠️ 今天沒有資料（可能非交易日或 API 無回應）。輸出空檔案並成功結束。")
        with open(args.out, "w", newline="", encoding="utf-8-sig") as f:
            w = DictWriter(f, fieldnames=["code","name","buy","sell","net"])
            w.writeheader()
        sys.exit(0)

    # 買超用 net 由大到小；賣超則相反（依你原本需求可再調）
